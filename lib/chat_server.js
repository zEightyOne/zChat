...
function joinRoom(socket, room) {
	...
	if (usersInRoom.length > 1) {
		var usersInRoomSummary = 'System: Users currently in ' + room + ': ';
...
function handleMessageBroadcasting(socket) {
	socket.on('message', function (message) { //LISTENER eg. message = {room: 'Lobby', text: 'Hello Ladies...'}
		var message_object = {text: nickNames[socket.id] + ': ' + message.text };
...
//// For Mocha POC - remove

const db = [];

exports.clear = () => { db.length = 0};
